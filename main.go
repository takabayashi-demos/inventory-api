...
func simulateInventoryChanges() {
	skus := []string{"SKU-001", "SKU-002", "SKU-003", "SKU-004", "SKU-005", "SKU-006", "SKU-007", "SKU-008"}
	for {
		time.Sleep(time.Duration(5+rand.Intn(10)) * time.Second)
		sku := skus[rand.Intn(len(skus))]
		if p, ok := inventory[sku]; ok {
			...
			log.Printf("Inventory update: %s stock adjusted by %d (new: %d)", sku, change, p.Stock)
		}
	}
}
